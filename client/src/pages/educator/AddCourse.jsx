...
const AddCourse = () => {
  ...
  const handleChapter = (action, chapterId) => {
    if (action === 'add'){
      const title = prompt('Vui lòng nhập tên chương:');
      if(title) {
        const newChapter = {
          chapterId: uniqid(),
          chaperTitle: title,
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-md w-full text-gray-500">
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            className="outline-none md:py-2.5 py-2 px-3 rounded border-gray-500"
            required
          />
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            min={0}
            max={100}
            className="outline-non md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500"
            required
          />
        </div>
        ...
        <div>
              ...
              <div className="flex justify-between items-center p-4 border-b">
                <div className="flex items-center">
                  ...
                  <span className="font-semibold">
                    {chapterIndex + 1} {chapter.chaperTitle}
                  </span>
                </div>
                ...
                <div className="p-4">
                  {chapter.chapterContent.map((lecture, lectureIndex) => (
                    <div key={lectureIndex} className="flex justify-between items-center mb-2">
                      <span>{lectureIndex + 1} {lecture.lectureTitle} - {lecture.lectureDuration} phút - <a href={lecture.lectureUrl} target="_blank" className="text-blue-500">Link</a> - {lecture.isPreviewFree ? 'Miễn phí' : 'Thanh toán'}</span>
          ...
          <div className="flex justify-center items-center bg-pink-100 p-2 rounded-lg cursor-pointer" onClick={() => handleChapter('add')}>
          + Thêm chương
          </div>
        ...
        <button type="submit" className="bg-pink-500 text-white w-max py-2.5 px-8 rounded my-4">THÊM</button>
      </form>
    </div>
  );
};
